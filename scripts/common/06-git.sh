...
# Simple git clone and cd into directory
sclone() {
	URL=$1
	if [ -z "${URL}" ]; then
        echo "Usage $0"
...
# Get short URL for a GitHub repository
git_url_shortener(){
...
# Fetch all remote branches and update local repo
...
# Checkout to master branch and pull latest changes
git_master () {
...
# Checkout to develop branch and pull latest changes
git_develop () {
